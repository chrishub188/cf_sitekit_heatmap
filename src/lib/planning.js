...
import { PLANNING_BOUNDARY, PLANNING_DESIGN, PLANNING_RESTRICTION_COLORS } from '$lib/style.js';

/**
 * @typedef {{
 *   id: string, url: string, number: number, label: string,
 *   group: 'restriction' | 'design', boundary: boolean,
...
function isProjected(geojson) {
	...
	if (typeof name === 'string') return name.includes('25832');
	let first = geojson.features?.find((/** @type {any} */ f) => f.geometry)?.geometry.coordinates;
...
export function loadPlanningLayer(url) {
	...
	if (!pending) {
		pending = fetch(url)
			...
			.then((geojson) => {
				const projected = isProjected(geojson);
